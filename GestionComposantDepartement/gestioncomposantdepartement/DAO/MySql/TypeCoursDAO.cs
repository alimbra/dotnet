...
namespace gestioncomposantdepartement.DAO.MySql
{
    class TypeCoursDAO:DAO
    {
        ...
        public List<TypeCours> findAll()
        {
            commande.CommandText = @"SELECT * FROM projetgestiondepartement.TypeCours ";
            MySqlDataReader msdr = commande.ExecuteReader();
            List<TypeCours> typeCours = new List<TypeCours>(msdr.FieldCount);
            while (msdr.Read()) { typeCours.Add(objetTypeCours(msdr)); }
            msdr.Close();
            return typeCours;
        }
        ...
        public List<TypeCours> findBy(Dictionary<string, string> contraintes)
        {
            try
            {
                int i = 0;
                StringBuilder sb = new StringBuilder("");
                sb.Append(@"SELECT * FROM projetgestiondepartement.TypeCours ");
                foreach (KeyValuePair<string, string> c in contraintes)
                {
                    if (i == 0) sb.Append("  WHERE projetgestiondepartement.TypeCours." + c.Key + " ='" + c.Value + "'");
                    sb.Append("  AND projetgestiondepartement.TypeCours." + c.Key + " = '" + c.Value + "'");
                    i++;
                }
                ...
                MySqlDataReader msdr = commande.ExecuteReader();
                List<TypeCours> lc = new List<TypeCours>();
                while (msdr.Read()){ lc.Add(this.objetTypeCours(msdr));}
                msdr.Close();
